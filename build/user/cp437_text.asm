...
;	user/cp437_text.c: 46: if(str[charLen] < 32)
	cp	a, #0x20
	jrnc	00105$
;	user/cp437_text.c: 47: textInc = 32;
	ldw	x, #0x0020
	jra	00121$
00105$:
;	user/cp437_text.c: 48: else if(str[charLen] > 127)
	cp	a, #0x7f
	jrule	00102$
;	user/cp437_text.c: 49: textInc = 127;
	ldw	x, #0x007f
	jra	00121$
00102$:
;	user/cp437_text.c: 51: textInc = str[charLen];
	clrw	x
	ld	xl, a
;	user/cp437_text.c: 52: for(charW=0;charW<8;charW++)
00121$:
	sllw	x
	sllw	x
	sllw	x
	...
	clr	a
00112$:
;	user/cp437_text.c: 54: buff[charW] = cp437_font[textInc*8+charW];
	clrw	y
	ld	yl, a
	addw	y, (0x16, sp)
	clrw	x
...
;	user/cp437_text.c: 52: for(charW=0;charW<8;charW++)
	...
	cp	a, #0x08
	jrc	00112$
;	user/cp437_text.c: 56: for (add = 0; add < 8; add++)
	clr	(0x09, sp)
00114$:
;	user/cp437_text.c: 58: text.data[offSet+add] = buff[add];
	ld	a, (0x11, sp)
	add	a, (0x09, sp)
	clrw	x
	ld	xl, a
...
;	user/cp437_text.c: 56: for (add = 0; add < 8; add++)
	...
	cp	a, #0x08
	jrc	00114$
;	user/cp437_text.c: 60: charLen++;
	ldw	x, (0x0c, sp)
	incw	x
	ldw	(0x0c, sp), x
;	user/cp437_text.c: 61: offSet +=8;
	ldw	x, (0x10, sp)
	addw	x, #0x0008
	ldw	(0x10, sp), x
	jra	00109$
00116$:
	addw	sp, #25
	ret
;	user/cp437_text.c: 65: void TEXT_Manage(void)
;	-----------------------------------------
;	 function TEXT_Manage
;	-----------------------------------------
_TEXT_Manage:
	sub	sp, #4
;	user/cp437_text.c: 75: if(TIMER_CheckTimeMS(&text.tick, 100) == 0)
	ldw	x, #_text+0
	ldw	(0x03, sp), x
	ldw	x, (0x03, sp)
	incw	x
	...
	tnz	a
	jrne	00105$
;	user/cp437_text.c: 77: DrawEx(text.scrollCnt, text.data);
	ldw	x, (0x03, sp)
	addw	x, #0x0009
	ldw	y, (0x03, sp)
	ld	a, (y)
	...
	call	_DrawEx
	addw	sp, #3
;	user/cp437_text.c: 78: Screen();
	call	_Screen
;	user/cp437_text.c: 79: if(++text.scrollCnt >= text.dataLen)
	ldw	x, (0x03, sp)
	ld	a, (x)
	inc	a
	ld	(x), a
	...
	cpw	x, (0x01, sp)
	jrc	00105$
;	user/cp437_text.c: 81: text.scrollCnt = 1;
	ldw	x, (0x03, sp)
	ld	a, #0x01
	ld	(x), a
00105$:
